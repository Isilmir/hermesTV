...
export default {
  ...
  async mounted(){
	...
	let players
	try{
	players = await axios.get('http://localhost:5000/getPlayers_for_printform',//'https://blooming-refuge-12227.herokuapp.com/getBjzi',//'http://192.168.0.148:5000/getBjzi', 
		{
			headers: {
	...
	players.data.filter(el=>el.count<8).map(el=>{this.tabs.push({id:el.id,name:el.name,label:el.name,disabled:true,isPlayer:true})})
...
//document.getElementById('printform-wrapper').style.display='none';
	...
	async testFaces(){
		...
		let faces;
		try{
			faces = await axios.get('http://localhost:5000/faces',//'https://blooming-refuge-12227.herokuapp.com/faces', //'http://192.168.0.148:5000/faces',
			{
				headers: {
				  'Content-Type': 'application/json',
				  'Authorization':`Bearer ${localStorage.getItem('jwt').replace(/"/g,'')}`
		...
		for (let tab_=0;tab_<this.tabs.length-1;tab_++){
			...
			let face;
			try{
				face = await axios.get(`http://localhost:5000/faces/${this.tabs[tab_].img}?sex=${this.sex}`,//`https://blooming-refuge-12227.herokuapp.com/faces/${this.tabs[tab_].img}?sex=${this.sex}`, //`http://192.168.0.148:5000/faces/${this.tabs[tab_].img}?sex=${this.sex}`,
				{
					headers: {
					  'Content-Type': 'application/json',
					  'Authorization':`Bearer ${localStorage.getItem('jwt').replace(/"/g,'')}`
